...
=======
import {
  demoAlerts, demoBudget, demoChatResponse, demoDashboard, demoGoals, demoHoldings,
  demoTransactions,
} from './demoData';
>>>>>>> Stashed changes
...
=======
  ...
  isUnavailable: (error: unknown) => axios.isAxiosError(error) && !error.response,
>>>>>>> Stashed changes
};

export const dashboardApi = {
  get: () => client.get<DashboardData>('/dashboard').then((r) => r.data)
    .catch((error) => { if (authApi.isUnavailable(error)) return demoDashboard; throw error; }),
};

export const transactionsApi = {
  list: () => client.get<Transaction[]>('/transactions').then((r) => r.data)
    .catch((error) => { if (authApi.isUnavailable(error)) return demoTransactions; throw error; }),
  ...
  get: () => client.get<{ budget: BudgetLine[]; total_budget: number }>('/budget')
    ...
    .catch((error) => {
      if (authApi.isUnavailable(error)) {
        return { budget: demoBudget, total_budget: demoBudget.reduce((sum, item) => sum + item.limit, 0) };
      }
      throw error;
    }),
};
...
export const goalsApi = {
  list: () => client.get<{ goals: Goal[] }>('/goals').then((r) => r.data.goals)
    .catch((error) => { if (authApi.isUnavailable(error)) return demoGoals; throw error; }),
};
...
export const investmentsApi = {
  get: () => client.get<{
    ...
    .catch((error) => {
      if (authApi.isUnavailable(error)) {
        const totalInvested = demoHoldings.reduce((sum, item) => sum + item.invested, 0);
        const totalCurrent = demoHoldings.reduce((sum, item) => sum + item.current, 0);
        return { holdings: demoHoldings, total_invested: totalInvested, total_current: totalCurrent, unrealised_gain: totalCurrent - totalInvested };
      }
      throw error;
    }),
};
...
export const alertsApi = {
  list: () => client.get<Alert[]>('/alerts').then((r) => r.data)
    .catch((error) => { if (authApi.isUnavailable(error)) return demoAlerts; throw error; }),
...
export const aiApi = {
  chat: (message: string) =>
    client.post<ChatResponse>('/ai/chat', { message }).then((r) => r.data)
      .catch((error) => { if (authApi.isUnavailable(error)) return demoChatResponse(message); throw error; }),
};
